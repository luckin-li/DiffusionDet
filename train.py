...
# ================= 自定义加载函数 =================
def load_coco_json_with_custom_fields(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    """
    Standard load_coco_json drops custom fields in 'images' list.
    This function loads them back (specifically 'aug_file_names').
    """
    # 1. 先使用标准加载器加载基础信息
    ...
    try:
        ...
        id_to_img_info = {img['id']: img for img in coco_content['images']}

        # 3. 将自定义字段合并回 dataset_dicts
        count_aug = 0
        for d in dataset_dicts:
            img_id = d['image_id']
            if img_id in id_to_img_info:
                info = id_to_img_info[img_id]
                # 检查并注入 aug_file_names
                if 'aug_file_names' in info:
                    d['aug_file_names'] = info['aug_file_names']
                    count_aug += 1

        print(f"Loaded {count_aug} images with 'aug_file_names' from {json_file}")

    except Exception as e:
        print(f"Warning: Failed to inject custom fields for {json_file}. Error: {e}")
...
def register_dataset():
    ...
    for key, (image_root, json_file) in PREDEFINED_SPLITS_DATASET.items():
        ...
        DatasetCatalog.register(key,
                                lambda j=json_file, i=image_root, n=key: load_coco_json_with_custom_fields(j, i, n))
...
def plain_register_dataset():
    # 同样建议在这里使用自定义 loader，如果 coco_my_train 也包含增广数据
    DatasetCatalog.register("coco_my_train",
                            lambda: load_coco_json_with_custom_fields(TRAIN_JSON, TRAIN_PATH, "coco_my_train"))
    ...
    DatasetCatalog.register("coco_my_val", lambda: load_coco_json_standard(VAL_JSON, VAL_PATH, "coco_my_val"))
